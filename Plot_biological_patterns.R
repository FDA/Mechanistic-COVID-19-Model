...
seedi=550
dataSi2=c()
for (seedi in seq(501,750,2)) {
if (caseMS=="severReady")	 {
	#readlseeed is 2962+1000
	longname=paste0(seedi,"_Virus2_0o5x_q90_1testT104_twouc_xunSM0.39_0.07_0.7_1.15_L0.7_1.15_number_p500_daybase_parnum_init4000_parnum_init_2200")
	shortname="Both_Final2_using_differnt_damagepercentage"
	previous_step=sprintf("../test/%s/nnn_80pop_500step_100pmax_Dmage0o6_ty3_EComIFN/%s/1/Bioout_plot_paperall_severReady.csv",shortname,longname)
	if (!file.exists(previous_step)) {print(seedi);next}else{
		print("yes---")
		print(seedi)
		dataSi=read.csv(previous_step,header = TRUE,sep = ",")
		dataSi1=dataSi[seq(0, nrow(dataSi), 10), ]
		dataSi1$seedi=seedi
		dataSi2=rbind(dataSi2,dataSi1)
	}
}
}
...
All_adm_t=c()
#colnames(allpar)=BCinf_pars0$V1
#population------------------
DamP=10
allpar=par_rand1

#write.csv(allpar,sprintf("suplimantary_table_q95/%s_allpar.csv",caseMS))
...
#-------------------------
	...
	initstates=states
#initstates["FreeInfluenza"]=as.numeric(pars_Changed1$V2[pars_Changed1$V1=="FreeInfluenza"])
	#initstates["FreeInfluenza"]=CSpar["FreeInfluenza"]
...
# READ EXPERIMENTAL DATA OF COVID-------------------------------
	...
	if (caseMS=="mildReady") {
		...
		#VIRUS$time=(VIRUS$Rx*24+incDay*24)
		addday=7
		adddaysim=incDay
...
#addday=7
	...
	IL6$y1mean=IL6$y1mean*75/90 #Dr Li email
	IL6$y1min=IL6$y1min*75/90 #Dr Li email
	IL6$y1max=IL6$y1max*75/90 #Dr Li email
	#plasma=nasal/75
#Dr li ask to plot based on plasma. so we have to devided IL6 by 75 
...
#experiment % (here exp is for lyph but we assume that the deacresing of Tnaive ir same as lymph and so the simulation tnaive % deacrese should be same as lymph)
...
#--- load ODE model, states, and parameters
	...
	pars["unnamed"]=1
#--------------------------------
#if (ip==0 & i1==1) {write.csv(pars,paste0(caseMS,"_parsip0i11.csv"))}
...
#out_plot_paper$NoEpthGen=-K11*FreeInfluenza*EpitheliumLung+K27*ResistantEpithelium-K26*IFNA1*EpitheliumLung-K288*IL6*EpitheliumLung
...
#	Epitheliumwithreplicatedinfluenza=0,FreeInfluenza=0.043,Epitheliumwithinfluenzaparticle=0,EpitheliumLung=4e+08,InfectedAPC=0,IL6=0,ResistantEpithelium=0,IFNA1=0,TcLymphnode=0,ActivatedAPCLymphnode=0,A=100000,
#	EpiKilledbyIFN=0,EpiKilledbyIL6=0,EpiKilledbyTc=0,EpiKilledbyVirus=0
...
#	daybase=round(adddaysim*24)+cutday*24
	reme1=daybase%%2
...
#for bological band calculation
	...
	#out_plot_paper=out_plot_paper[out_plot_paper[,"time"]<=29*24,]
...
#Bioout_plot_paperall_timeend=Bioout_plot_paperall[Bioout_plot_paperall$time==length(Bioout_plot_paperall$time),]
#maxDam=(4E8-Bioout_plot_paperall$ResistantEpithelium-Bioout_plot_paperall$EpitheliumLung)/4E8*100
#if (i1==1) {write.csv(cross5per1,sprintf("%s_i1is1_initial_%s.csv",outdir2,caseMS))}
	...
	if (ip!=0) {
		...
		#for (ty in out_plot1_paper_ip$time) {
		#cross5per=which(out_plot1_paper_ip_m1[ty+1]>recovery_threshold & out_plot1_paper_ip_m1[ty]<recovery_threshold)
		#}
		#cross5per=which(abs(out_plot1_paper_ip$Damageper-recovery_threshold)==min(abs(out_plot1_paper_ip$Damageper-recovery_threshold)))
...
#		qqqqqqqqqqqq
		...
		#if (is.na(cross5pert0) & i1>1) {cross5pert0=10000}
		if (is.na(cross5pert0)) {cross5pert0=10000}
#		if (is.na(cross5pert0)) {cross5pert0=10000}
#		if (is.na(cross5pert0) & i1>1) {cross5pert0=10000}
#		if (min(out_plot1_paper_ip$Damageper)<recovery_threshold) {cross5pert0=-10000}
#		if ((cross5pert0)) {cross5pert0=10000}
		...
		#if (length(cross5pert0)==0 & i1>0) {cross5pert0=10000}
		
#		print(cross5pert0)
#		print("print(cross5pert0)")
		#out_plot1_paper$cross5per[out_plot1_paper$ip==ipa]=cross5per
		#cross5per1=rbind(cross5per1,c(unique(out_plot1_paper_ip$ip),ipa,cross5pert0))
#		cross5per1=rbind(cross5per1,c(ip,ipun,cross5pert0,min(eventtimes),damageD28))
...
if (i1==1) {write.csv(Bioout_plot_paperall,sprintf("%sBioout_plot_paperall_%s.csv",outdir2,caseMS))}
#sprintf("%s_i1is2_cross5per1_%s.csv",outdir2,caseMS)

source("plotexampleAB.R")
...
#if ( i1==1) {write.csv(out_plot1_paper,"1out_plot1_paper.csv")}
#if ( i1==2) {write.csv(out_plot1_paper,"2out_plot1_paper.csv")}
	
#--------regular pars
#pars[pidx!=0]<-pars_calc[pidx]
#pars["starttime"]<-unclass(as.POSIXct(strptime(date(),"%c")))[1]
#pars["timeout"]<-5
#try({out_plot <- dede(initstates, timepoints, "derivs", pars, dllname=modelname,initfunc="initmod",rtol=1e-3,atol=1e-6,nout=2,)});		
...
#for (ti1 in timepoints) {
	for (ti1 in seq(0,(max(out_plot1_paper[,"time"])-24),2)) {
		...
		#tq=(lapply(tout_plot1,quantile,probs=c(0,1),na.rm=TRUE))
	...
	#finding examples that Zhihua said by initial infection comparision
	
	
	
#for (ti1 in seq(0,(max(Bioout_plot_paperall[,"time"])),2)) { #2 not 1 or odd
	for (ti1 in seq(0,max(out_plot1[,"time"]),2)) { #2 not 1 or odd
		...
		#BREcell=rbind(BREcell,c(ti1,tq$REcell))
...
Damageper1=c()
#colnames(cross5per1)=c("ipr","ip","Recoveredt")
#cross5per1=data.frame(cross5per1)
cross5per_step2=cross5per1
if (i1!=2) { #in treated case it can be 0 
	cross5per1=cross5per1[cross5per1$Recoveredt>0,]
}
maxtime=29*24#11*24
#cross5per1=cross5per1[cross5per1$Recoveredt<=(maxtime),]
...
#if (i1==1 & caseMS=="mildReady") {maxRecv=0.80}
#if (i1==2 & caseMS=="mildReady") {maxRecv=0.92}
#if (i1==1 & caseMS=="severReady") {maxRecv=0.71}
#if (i1==2 & caseMS=="severReady") {maxRecv=0.85}
if (i1==0) {
	if (caseMS=="mildReady") { 
		...
		#d_diff=diff(Mild_Control_meanstep$opt)
		
	}
#if (i1==2 & caseMS=="mildReady") {maxRecv=0.841}
	if ( caseMS=="severReady") { 
...
#cross5per1$Roundedday%in%Sever_Control$day
...
#		d_diff=c(0,d_diff)
	...
	#cross5per1$NRoundedday[is.na(cross5per1$NRoundedday)]=20000
	...
	if (simpleunc=="yes") {
		...
		set.seed(seedn)
		#Uc1=round(xunc*(ucov_PN-1))
#		Uc2=(ucov_PN-1)-Uc1
#	xunc=(1-max(Sever_Control$opt)) # remove it from prevus
		Uc2=round(xunc*number_p)
		Uc1=round(number_p-length(wantip1)-Uc2)*0
		deadman=0
#		if (caseMS=="severReady") {deadman=(15*5+40)*2}
...
#ucov_P=cross5per1$ip[cross5per1$NRoundedday>c1day & cross5per1$NRoundedday<c2day]
		...
		#for sever
	if (caseMS=="severReady"){
#		ucov_P2=cross5per1$ip[cross5per1$NRoundedday>50 & cross5per1$minevent>550 & cross5per1$damageD11>=DamP]
		ucov_P2=cross5per1$ip[cross5per1$NRoundedday>50  & cross5per1$damageD11>=15]
		
		if (seedn>250) {
		ucov_P2=cross5per1$ip[ cross5per1$damageD11>=15]
	}
	
	
	if (seedn>500) {
		ucov_P2=cross5per1$ip[cross5per1$NRoundedday>50  & cross5per1$damageD11>=20]
	}
	if (seedn>750) {
		ucov_P2=cross5per1$ip[ cross5per1$damageD11>=20]
	}
	
	if (seedn>1000) {
		ucov_P2=cross5per1$ip[cross5per1$NRoundedday>30  & cross5per1$damageD11>=15]
	}
	
	if (seedn>1250) {
		ucov_P2=cross5per1$ip[cross5per1$NRoundedday>30  & cross5per1$damageD11>=20]
	}
#		ucov_P2=cross5per1$ip[cross5per1$NRoundedday>50 ]
		
	}
	if (caseMS=="mildReady"){
		#for mild is 
		ucov_P2=cross5per1$ip[cross5per1$NRoundedday>30 ]
	}
	...
	next #i1=0 is uptohere and then Next i1=1
}
#for (ita in seq(1,(maxtime))) {
#	Damageper0=sum(cross5per1$Recoveredt<=ita)/(number_p+deadpop)*100
#	Damageper1=rbind(Damageper1,c(ita,Damageper0))
#}
...
#write.csv(BTnaiveq1,sprintf("BTnaiveq1.csv",outdir2))
p<-ggplot()
...
#p<-p+ylab(paste0("Tnaive Drop(%)"))+ylim(0,100)
...
#IL6$y1max=IL6$y1min[IL6$y1max>0]
...
#p3<-p3+ylab(paste0("IL6","_",EC))

#FreeInfluenza
#fracdata_plot=fracdata
...
#BIL6q1$qmin=log10(out_plot2$FreeInfluenza)

#out_plot1_paper$FreeInfluenza=log10(out_plot1_paper$FreeInfluenza)
#if (i1==1) {out_plot1i1=out_plot1}
#if (i1==2) {out_plot1i2=out_plot1}
#if (i1==2) {

p1<-ggplot()
#p1<-p1+geom_point(data=out_plot1, aes(x=time, y=FreeInfluenza), size=line_size, alpha=.5, color="red")
...
#p1<-p1+geom_line(data=out_plot1, aes(x=time, y=FreeInfluenza), size=0.85, alpha=0.8,linetype = "solid")
#p1<-p1+geom_line(data=out_plot1i2, aes(x=time, y=FreeInfluenza), size=0.85, alpha=0.8,linetype = "dashed",col="black")
p1<-p1+geom_ribbon(data=BFreeInfluenzaq1, aes(x=time, ymin=(qmin),ymax=(qmax)),alpha=0.3)

#write.csv(out_plot1,paste0(caseMS,EC,".csv"))
#p1=p1 + geom_hline(yintercept=log10(37803.93), linetype="dashed", color = "black")#+
	#	geom_vline(xintercept=st_day*24, linetype="dashed", color = "brown")
...
p1<-p1+ylab("Virus Titer \n (log[copy number/ml])")+ylim(0,9)+xlim(0,576)+theme(axis.text=element_text(size=numsiz),axis.title.y=element_text(size=labsiz,face="bold"))+theme(axis.title.x = element_blank(), axis.text.x = element_blank())

#}
#INFAnew
#IFNAalpha0<-read.csv(paste0("Data_Covid/IFNAalpha/","IFNA_Fig1A",".csv"),sep = ",") #read data
#IFNAalpha=data.frame(cbind(unique(IFNAalpha0$Day),unique(IFNAalpha0$INFA_mean_pgperml))) #Data in each day 
#colnames(IFNAalpha)=c("Day","INFA_mean_pgperml") #paper was based on fg/ml , i changed it
...
#IFNAalpha$Time=(IFNAalpha$Day+addday)*24    #addday for mild case is 7 days. days after infection
...
#IFA_sever$Day=IFA_sever$time
IFA_sever$IFN_S=IFA_sever$IFN_S*12*.1
#------end sever-------

#2)	We need a scaling factor. In your file the peak (8.6 pg/ml) is Day 4 after symptom, or ~day 10 after infection. In the best pars I got, on ~day 10 IFN is ~100 pg/ml for mild patients. So maybe we can use a scaling factor of 12?
#mild--
IFNAalpha=IFNAalpha[IFNAalpha$Day<23,] 		#how many days after onset is keep (i used 15day) 
...
#ggsave(paste0("_LOSH.png"), p5, width=6, height=8)
#ccccccccccccc
p6<-ggplot()
...
#p5<-ggplot(out_plot1, aes(x=time,y=Epitheliumwithreplicatedinfluenza))
##p4<-p4+geom_point(data=IL6, aes(x=time, y=y1mean), size=line_size, alpha=.5, color="green")
#p5<-p5+geom_line(data=out_plot1, aes(x=time, y=Epitheliumwithreplicatedinfluenza), size=0.85, alpha=0.8,linetype = "solid")
#p5<-p5+ylab(paste0("Epitheliumwithreplicatedinfluenza","_",EC))
#
#
#p6<-ggplot(out_plot1, aes(x=time,y=EpitheliumLung))
##p4<-p4+geom_point(data=IL6, aes(x=time, y=y1mean), size=line_size, alpha=.5, color="green")
#p6<-p6+geom_line(data=out_plot1, aes(x=time, y=EpitheliumLung), size=0.85, alpha=0.8,linetype = "solid")
#p6<-p6+ylab(paste0("EpitheliumLung","_",EC))

#add Non-sever IgM and IgG ---------------------------------------------
#1) NS_IgM
if (caseMS=="severReady") {
...
library(grid)
#pall <- grid.arrange(grobs=plotlist,ncol=2,top = "non-sever")
#pall=grid.arrange(arrangeGrob(p,p4,p5,p6,p7,p8, ncol=2, nrow=3),
#pall=grid.arrange(arrangeGrob(p,p4,p5,p7,p9,p10, ncol=2, nrow=3),
#		pall=grid.arrange(arrangeGrob(p,p4,p1,p5,p9,p10, ncol=1, nrow=6),
				
#		arrangeGrob(p1, ncol=1, nrow=1), heights=c(10,.1), widths=c(2,1),bottom = textGrob(paste0("Patient: ",caseMS,"\n","Model: 3delay","\n","Changed Pars: ",pars_Changed_path,"viruscytokines_pars.txt"), gp = gpar(fontsize = 6.5)))
...
#system(paste0("mkdir -p ","figs/",outfolder))
...
#superimpose the IFN and Virus
PVIF<-ggplot()
#PVIF<-PVIF+geom_point(data=out_plot1, aes(x=time, y=FreeInfluenza), size=line_size, alpha=.5, color="red")
PVIF<-PVIF+geom_line(data=out_plot1, aes(x=time, y=FreeInfluenza), size=line_size, alpha=1,linetype = "solid")
#PVIF<-PVIF+geom_point(data=VIRUS, aes(x=time, y=FreeInfluenza1), size=point_size, alpha=1, color="black")
#PVIF<-PVIF+geom_line(data=out_plot1, aes(x=time, y=FreeInfluenza), size=0.85, alpha=0.8,linetype = "solid")
#PVIF<-PVIF+geom_line(data=out_plot1i2, aes(x=time, y=FreeInfluenza), size=0.85, alpha=0.8,linetype = "dashed",col="black")
#PVIF<-PVIF+geom_ribbon(data=BFreeInfluenzaq1, aes(x=time, ymin=(qmin),ymax=(qmax)),alpha=0.3)

#write.csv(out_plot1,paste0(caseMS,EC,".csv"))
#PVIF=PVIF + geom_hline(yintercept=log10(37803.93), linetype="dashed", color = "black")#+
#	geom_vline(xintercept=st_day*24, linetype="dashed", color = "brown")
...
PVIF<-PVIF+ylab("Virus Titer (log[copy number/ml]")+ylim(0,9)+xlim(0,576)+theme(axis.text=element_text(size=numsiz),axis.title.y=element_text(size=13,face="bold"))+theme(axis.title.x = element_blank(), axis.text.x = element_blank(),panel.grid.minor = element_blank(),panel.grid.major = element_blank())

#ssss
PVIF1<-ggplot()
#if (caseMS=="mildReady") {
#	PVIF1<-PVIF1+geom_point(data=IFNAalpha, aes(x=Time, y=INFA_mean_pgperml), size=point_size, alpha=1, color="black",shape=1)
#}
#if (caseMS=="severRead") {
#	PVIF1<-PVIF1+geom_point(data=IFNAalpha, aes(x=Time, y=1), size=point_size, alpha=1, color="black",shape=1)
#}
lim=c(-5,200)
PVIF1<-PVIF1+geom_line(data=out_plot1, aes(x=time, y=IFNA1), size=line_size, alpha=1,linetype = "solid",color="darkgray")
#PVIF1<-PVIF1+geom_ribbon(data=BIFNA1q1, aes(x=time, ymin=qmin,ymax=qmax),alpha=0.3)
...
iPVIF<-ggplot()
#iPVIF<-iPVIF+geom_point(data=expINF, aes(x=V1, y=V2), size=line_size, alpha=.5, color="red")
...
iPVIF<-iPVIF+ylab("Virus Titer (log[copy number/ml]")+ylim(0,6)+xlim(0,192)+theme(axis.text=element_text(size=numsiz),axis.title.y=element_text(size=13,face="bold"))+theme(axis.title.x = element_blank(), axis.text.x = element_blank(),panel.grid.minor = element_blank(),panel.grid.major = element_blank())
...
ppk=ggplot() +
		...
		#geom_point(data=data_RMD,aes(x=time, y=M, group=dose,color=as.factor(dose)))+
		#guides(color=guide_legend(override.aes=list(shape=c(NA,16),linetype=c(1,0))))
		#scale_color_manual(values=c("#004E00", "#33FF00", "#FF9966", "#3399FF", "#FF004C","purple")) +
		#scale_shape_manual(values=c(1:5, 32)) +scale_y_continuous(trans='log10')+
...
if (i1==2) {
	...
	p1<-ggplot()
#p1<-p1+geom_point(data=out_plot1, aes(x=time, y=FreeInfluenza), size=2, alpha=.5, color="red")
#p1<-p1+geom_line(data=out_plot1, aes(x=time, y=FreeInfluenza), size=0.85, alpha=0.8,linetype = "solid")
	#p1<-p1+geom_point(data=VIRUS, aes(x=time, y=FreeInfluenza1), size=2, alpha=.5, color="red")
...
#write.csv(out_plot1,paste0(caseMS,EC,".csv"))
	#p1=p1 + geom_hline(yintercept=log10(37803.93), linetype="dashed", color = "green")#+
	#geom_vline(xintercept=st_day*24, linetype="dashed", color = "brown")
	...
	ppk1=ggplot() +
#			geom_line(data=out_plot1i1,aes(x=time, y=(4E8-ResistantEpithelium-EpitheliumLung)/4E8*100),color="red",linetype = "solid") +
#			geom_line(data=out_plot1i2,aes(x=time, y=(4E8-ResistantEpithelium-EpitheliumLung)/4E8*100),color="blue",linetype = "solid") +
			geom_ribbon(data=REcell, aes(x=time/24, ymin=(4E8-qmax)/4E8*100,ymax=(4E8-qmin)/4E8*100) ,alpha=0.8,fill="gray")+
#			geom_ribbon(data=REcell_NoRMD, aes(x=time/24, ymin=(4E8-qmax)/4E8*100,ymax=(4E8-qmin)/4E8*100) ,alpha=0.3,fill="gray")+
...
ppkjc1=ggplot() +
		#geom_line(data=out_plot1i1,aes(x=time, y=(4E8-ResistantEpithelium-EpitheliumLung)/4E8*100),color="red",linetype = "solid") +
		#geom_line(data=out_plot1i2,aes(x=time, y=(4E8-ResistantEpithelium-EpitheliumLung)/4E8*100),color="green",linetype = "dashed") +
		geom_ribbon(data=REcell_NoRMD, aes(x=time/24, ymin=(4E8-qmax)/4E8*100,ymax=(4E8-qmin)/4E8*100) ,alpha=0.8,fill="gray")+
		#geom_ribbon(data=REcell_NoRMD, aes(x=time, ymin=(4E8-qmax)/4E8*100,ymax=(4E8-qmin)/4E8*100) ,alpha=0.3,color="brown",fill="brown")+
		geom_hline(yintercept=5, linetype="dashed", color = "black")+
...
#optimum case total time course--------------------------------------------------
#if (ip ==0 & i1==1) {out_plot1_opt0Biolog_opt_alltime=out_plot_paper}
#if (ip ==0 & i1==2) {out_plot1_opt0_opt_alltime=out_plot_paper}
...
#optp1<-optp1+geom_ribbon(data=FreeInfluenzaq, aes(x=time, ymin=log10(qmin),ymax=log10(qmax)),alpha=0.3)
#optp1<-optp1+geom_ribbon(data=FreeInfluenzaq_NoRMD, aes(x=time, ymin=log10(qmin),ymax=log10(qmax)),alpha=0.3,color="brown",fill="brown")
	
#write.csv(out_plot1,paste0(caseMS,EC,".csv"))
#optp1=optp1 + geom_hline(yintercept=log10(37803.93), linetype="dashed", color = "green")#+
#geom_vline(xintercept=st_day*24, linetype="dashed", color = "brown")
...
#		geom_ribbon(data=REcell, aes(x=time, ymin=(4E8-qmax)/4E8*100,ymax=(4E8-qmin)/4E8*100) ,alpha=0.3)+
#		geom_ribbon(data=REcell_NoRMD, aes(x=time, ymin=(4E8-qmax)/4E8*100,ymax=(4E8-qmin)/4E8*100) ,alpha=0.3,color="brown",fill="brown")+
			theme(legend.position = "none") +
			labs(x ="time (hr)", y = "%Damaged Cell")+xlim(0,15*24)#xlim(0, (st_day+num_day)*24)
...
#totalkilled<- 4E8 - out_plot1[,"EpitheliumLung"] -out_plot1[,"ResistantEpithelium"]
	...
	totalkilled=1
	kia1<-ggplot()+
#p4<-p4+geom_point(data=IL6, aes(x=time, y=y1mean), size=2, alpha=.5, color="green")
...
#p4<-p4+geom_point(data=IL6, aes(x=time, y=y1mean), size=2, alpha=.5, color="green")
	...
	kia4<-ggplot()+
#p4<-p4+geom_point(data=IL6, aes(x=time, y=y1mean), size=2, alpha=.5, color="green")
#		geom_line(data=out_plot1_opt0Biolog_opt_alltime, aes(x=time, y=pars["K11"]*FreeInfluenza*EpitheliumLung ), size=0.85, alpha=0.8,linetype = "solid")+
#		geom_line(data=out_plot1_opt0_opt_alltime, aes(x=time, y=pars["K11"]*FreeInfluenza*EpitheliumLung ), size=0.85, alpha=0.8,linetype = "solid",color="green")+
...
#Historgram Virus and t administration----------------------------------------------------
...
#Viral titer at initial of administration
...
#subplot based on long and short
	sepday=12
...
#		geom_line(data=out_plot1i2LSH, aes(x=time, y=log10(FreeInfluenza),group=ip), size=0.85, alpha=0.8,linetype = "dashed",col="red")+
			...
			#geom_line(data=out_plot1i2LL, aes(x=time, y=log10(FreeInfluenza),group=ip), size=0.85, alpha=0.8,linetype = "dashed",col="green")+
			xlab(paste0("time (hr)"))+ylab("EpiKilledbyIFN")#+ggtitle(paste0(caseMS,": \n Black= without drug, Green= with drug"))#+xlim(0, (st_day+num_day)*24)
...
#			geom_ribbon(data=RC_D, aes(x=day, ymin=(min),ymax=(max)),alpha=0.3,color="blue",fill="blue")+
#			geom_ribbon(data=RC_C, aes(x=day, ymin=(min),ymax=(max)),alpha=0.3,color="orange",fill="orange")+
		...
		geom_errorbar(data=RC_C, aes(x=day, ymin=(min),ymax=(max)),color="indianred3",fill="indianred3",width=.3,
			...
			#geom_line(data=out_plot1i2LL, aes(x=time, y=log10(FreeInfluenza),group=ip), size=0.85, alpha=0.8,linetype = "dashed",col="green")+
			xlab(paste0("time(Day)"))+ylab("Recovered%")+ylim(0,105)+ggtitle(paste0(caseMS,"_",iT,"_seed",seedn))+#+xlim(0, (st_day+num_day)*24)
...
#			geom_line(data=RC_C_step, aes(x=day, y=opt ), size=1.5, alpha=1,linetype = "solid",col="green")+
			
			#geom_line(data=Damageper2RMD, aes(x=time/24, y=Recovered ), size=1.5, alpha=1,linetype = "solid",col="darkblue")+
			#geom_ribbon(data=RC_D, aes(x=day, ymin=(min),ymax=(max)),alpha=0.3,color="blue",fill="blue")+
		geom_errorbar(data=RC_C, aes(x=day, ymin=(min),ymax=(max)),width=.8,
				position=position_dodge(2),color="black",fill="black")+
			#geom_line(data=RC_D, aes(x=day, y=opt), size=0.85, alpha=0.8,linetype = "dashed",col="blue")+
#			geom_line(data=RC_C, aes(x=day, y=opt), size=0.65, alpha=0.8,linetype = "dashed",col="black")+
			geom_point(data=RC_C, aes(x=day, y=opt), size=1.5, alpha=1,color="black")+
			
			#geom_line(data=out_plot1i2LL, aes(x=time, y=log10(FreeInfluenza),group=ip), size=0.85, alpha=0.8,linetype = "dashed",col="green")+
			xlab(paste0("time(Day)"))+ylab("Recovered%")+ylim(0,100)+xlim(0,27)+ scale_x_continuous(breaks = seq(0, 27, by = 9))+#ggtitle(paste0(caseMS,"_",iT,"_seed",seedn))+#+xlim(0, (st_day+num_day)*24)
...
#	ggsave(paste0(outdir2,byplot,EC,"_","Recovery_Rec_JustCtrl1",caseMS,"_kill.png"), Rec_JustCtrl, width=3.57*1.25, height=1.38*1.25)
...
#			geom_line(data=Damageper2RMD, aes(x=time/24, y=Recovered ), size=1.5, alpha=1,linetype = "solid",col="deepskyblue")+
#		
#			geom_line(data=RC_C_step, aes(x=day, y=opt ), size=1.5, alpha=1,linetype = "solid",col="black")+
#			geom_line(data=RC_D_step, aes(x=day, y=opt ), size=1.5, alpha=1,linetype = "solid",col="green")+
#			
##			geom_ribbon(data=RC_D, aes(x=day, ymin=(min),ymax=(max)),alpha=0.3,color="blue",fill="blue")+
##			geom_ribbon(data=RC_C, aes(x=day, ymin=(min),ymax=(max)),alpha=0.3,color="orange",fill="orange")+
#			geom_errorbar(data=RC_D, aes(x=day-.5, ymin=(min),ymax=(max)),color="blue",fill="blue",width=.3,
#					position=position_dodge(.9),color="blue",fill="blue")+
#			geom_errorbar(data=RC_C, aes(x=day-.5, ymin=(min),ymax=(max)),color="indianred3",fill="indianred3",width=.3,
#					position=position_dodge(.9),color="indianred3",fill="indianred3")+
#			
#			geom_line(data=RC_D_meanstep, aes(x=day, y=opt), size=0.85, alpha=0.8,linetype = "dashed",col="blue")+
#			geom_line(data=RC_C_meanstep, aes(x=day, y=opt), size=0.85, alpha=0.8,linetype = "dashed",col="indianred3")+
#			#geom_line(data=out_plot1i2LL, aes(x=time, y=log10(FreeInfluenza),group=ip), size=0.85, alpha=0.8,linetype = "dashed",col="green")+
#			xlab(paste0("time(Day)"))+ylab("Recovered%")+ylim(0,105)+ggtitle(paste0(caseMS,"_",iT,"_seed",seedn))+#+xlim(0, (st_day+num_day)*24)
#			theme(axis.text=element_text(size=20),axis.title=element_text(size=labsiz,face="bold"))
#	
#	ggsave(paste0(outdir2,byplot,EC,"_","Recovery",caseMS,"_kill_Step.png"), Recs, width=8, height=8)
	
	
	
	Recnoctr<-ggplot()+
			
#			geom_line(data=Damageper2placebo, aes(x=time/24, y=Recovered ), size=1.5, alpha=1,linetype = "solid",col="black")+
...
#			geom_line(data=RC_D, aes(x=day, y=opt), size=0.85, alpha=1,linetype = "dashed",col="blue")+
#			geom_line(data=RC_C, aes(x=day, y=opt), size=0.85, alpha=0.8,linetype = "dashed",col="indianred3")+
#			xlab(paste0("time(Day)"))+ylab("Recovered%")+ylim(0,105)+ggtitle(paste0(caseMS,"_",iT,"_seed",seedn))+#+xlim(0, (st_day+num_day)*24)
